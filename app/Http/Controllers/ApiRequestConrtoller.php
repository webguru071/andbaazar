...
class ApiRequestConrtoller extends Controller
{
    ...
    public function products(){
        $products = Product::with(['category','inventory','itemimage'])->where('type','ecommerce')->get();//->where('id','>=',10)->get();
        // dd($products);
        $i = 0;
        foreach($products as $product){
            ...
            foreach($product->inventory as $variants){
                $data[$i]['variants'][strtolower($variants->color_name)][]  = [
                    "variant_id" => $variants->id,
                    "id" => $variants->id,
                    "sku" => $variants->seller_sku,
                    "size" => 's',
    ...
    public function getColors(){
        return response()->json(['yellow','green','white']);
    }
    public function getSizes(){
        //call model
        $sizes = Size::select('slug')->get()->toArray();
